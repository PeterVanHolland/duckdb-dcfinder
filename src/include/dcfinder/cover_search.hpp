...
struct CoverSearch {
	...
	static vector<DenialConstraint> FindMinimalDCs(const EvidenceSet &evidence_set, const PredicateSpace &pred_space,
	                                               double epsilon, idx_t num_tuples);

private:
	static void FindCover(vector<idx_t> &path, vector<pair<EvidenceBitset, idx_t>> &uncovered,
